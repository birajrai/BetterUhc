package me.birajrai.schematics;
...
public class UndergroundNether extends Schematic {
	...
	public void build(MainConfig cfg, World world){
		if (!canBePasted()){
			Bukkit.getLogger().severe("[BetterUHC] Worldedit not installed or nether schematic not found in 'plugins/BetterUhc/nether.schematic'. There will be no underground nether");
			return;
		}
		...
		for(int i = 1; i <= occurrences ; i++){
			...
			try {
				...
				Bukkit.getLogger().severe("[BetterUHC] Couldn't paste nether schematic at "+
