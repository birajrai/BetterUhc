...
import me.birajrai.schematics.Lobby;
import me.birajrai.schematics.UndergroundNether;
import me.birajrai.threads.ChunkLoaderThread;
import me.birajrai.threads.WorldBorderThread;
...
public class MapLoader {
	...
	public void createNewWorld(Environment env){
		...
		if (BetterUhc.getPlugin().getConfig().getBoolean("permanent-world-names", false)){
		...
		try{
			storage = FileUtils.saveResourceIfNotAvailable(BetterUhc.getPlugin(), "storage.yml");
	...
	public void setWorldsStartGame() {
		...
		if(config.get(MainConfig.BORDER_IS_MOVING)){
			...
			Bukkit.getScheduler().runTask(BetterUhc.getPlugin(), new WorldBorderThread(timeBeforeShrink, endSize, timeToShrink));
		}
	}
	...
	private void prepareWorld(World world, Difficulty difficulty, boolean healthRegen, boolean announceAdvancements, int borderSize) {
		...
		if (!announceAdvancements && BetterUhc.getVersion() >= 12){
	...
	public void generateChunks(Environment env){
		...
		if (PaperLib.isPaper() && PaperLib.getMinecraftVersion() >= 13){
			Bukkit.getScheduler().runTaskAsynchronously(BetterUhc.getPlugin(), chunkLoaderThread);
		}else {
			Bukkit.getScheduler().runTask(BetterUhc.getPlugin(), chunkLoaderThread);
		}
	}
