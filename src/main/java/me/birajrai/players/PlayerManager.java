...
import me.birajrai.customitems.GameItem;
...
import me.birajrai.events.UhcWinEvent;
...
import me.birajrai.threads.CheckRemainingPlayerThread;
import me.birajrai.threads.TeleportPlayersThread;
...
public class PlayerManager {
	...
	public void setAllPlayersEndGame() {
		...
		if(cfg.get(MainConfig.ENABLE_BUNGEE_SUPPORT) && cfg.get(MainConfig.TIME_BEFORE_SEND_BUNGEE_AFTER_END) >= 0){
			for(UhcPlayer player : getPlayersList()){
				Bukkit.getScheduler().runTaskAsynchronously(BetterUhc.getPlugin(), new TimeBeforeSendBungeeThread(this, player, cfg.get(MainConfig.TIME_BEFORE_SEND_BUNGEE_AFTER_END)));
			}
		}
	...
	public void randomTeleportTeams() {
		...
		Bukkit.getScheduler().runTaskLater(BetterUhc.getPlugin(), () -> GameManager.getGameManager().startWatchingEndOfGame(), delayTeleportByTeam + 20);
	...
	public void startWatchPlayerPlayingThread() {
		...
		Bukkit.getScheduler().runTaskLater(BetterUhc.getPlugin(), new CheckRemainingPlayerThread(GameManager.getGameManager()) , 40);
	}
	...
	public void sendPlayerToBungeeServer(Player player) {
		...
		player.sendPluginMessage(BetterUhc.getPlugin(), "BungeeCord", out.toByteArray());
	}
