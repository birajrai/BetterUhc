...
import me.birajrai.customitems.KitsManager;
...
import me.birajrai.game.handlers.*;
import me.birajrai.languages.Lang;
import me.birajrai.listeners.*;
...
import me.birajrai.scoreboard.ScoreboardLayout;
import me.birajrai.scoreboard.ScoreboardManager;
import me.birajrai.threads.*;
import me.birajrai.utils.FileUtils;
import me.birajrai.utils.NMSUtils;
...
public class GameManager{
	...
	public GameManager() {
		...
		statsHandler = new StatsHandler(BetterUhc.getPlugin(), config, mapLoader, scenarioManager);
	...
	public void loadNewGame() {
		...
		if(config.get(MainConfig.ENABLE_BUNGEE_SUPPORT)) {
			BetterUhc.getPlugin().getServer().getMessenger().registerOutgoingPluginChannel(BetterUhc.getPlugin(), "BungeeCord");
		}
	...
	public void startWaitingPlayers() {
		...
		Bukkit.getScheduler().scheduleSyncDelayedTask(BetterUhc.getPlugin(), new PreStartThread(this),0);
	}
	...
	public void startWatchingEndOfGame(){
		...
		Bukkit.getScheduler().runTaskAsynchronously(BetterUhc.getPlugin(), new ElapsedTimeThread(this, customEventHandler));
		Bukkit.getScheduler().runTaskAsynchronously(BetterUhc.getPlugin(), new EnablePVPThread(this));

		if (config.get(MainConfig.ENABLE_EPISODE_MARKERS)){
			Bukkit.getScheduler().runTaskAsynchronously(BetterUhc.getPlugin(), new EpisodeMarkersThread(this));
		}

		if(config.get(MainConfig.ENABLE_DEATHMATCH)){
			Bukkit.getScheduler().runTaskAsynchronously(BetterUhc.getPlugin(), new TimeBeforeDeathmatchThread(this, deathmatchHandler));
		}

		if (config.get(MainConfig.ENABLE_DAY_NIGHT_CYCLE) && config.get(MainConfig.TIME_BEFORE_PERMANENT_DAY) != -1){
			Bukkit.getScheduler().scheduleSyncDelayedTask(BetterUhc.getPlugin(), new EnablePermanentDayThread(mapLoader), config.get(MainConfig.TIME_BEFORE_PERMANENT_DAY)*20);
		}

		if (config.get(MainConfig.ENABLE_FINAL_HEAL)){
            Bukkit.getScheduler().scheduleSyncDelayedTask(BetterUhc.getPlugin(), new FinalHealThread(this, playerManager), config.get(MainConfig.FINAL_HEAL_DELAY)*20);
        }
	...
	public void loadConfig(){
		...
		try{
			File configFile = FileUtils.getResourceFile(BetterUhc.getPlugin(), "config.yml");
	...
	private void registerListeners() {
		...
		for(Listener listener : listeners){
			Bukkit.getServer().getPluginManager().registerEvents(listener, BetterUhc.getPlugin());
		}
	}
	...
	public void endGame() {
		if(gameState.equals(GameState.PLAYING) || gameState.equals(GameState.DEATHMATCH)){
			...
			Bukkit.getScheduler().scheduleSyncDelayedTask(BetterUhc.getPlugin(), new StopRestartThread(),20);
		}
	}
